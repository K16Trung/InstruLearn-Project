...
namespace InstruLearn_Application.DAL.Repository.IRepository
{
    ...
    {
        Task<Learner_Content_Progress> GetByLearnerAndContentItemAsync(int learnerId, int itemId);
        Task<List<Learner_Content_Progress>> GetByLearnerAndCourseAsync(int learnerId, int coursePackageId);
        Task<bool> UpdateWatchTimeAsync(int learnerId, int itemId, double watchTimeInSeconds, bool isCompleted = false);
